...
如果一个从者攻击一个因对肃正、无敌或回避而免受伤害的敌人，其np回收不会被蓝卡耐性提升所影响，而只会被蓝卡耐性降低影响。

进一步的测试表明任何负面的防御类buff在敌方免受伤害时正常生效，但是正面的防御类效果则会无效化并且不消耗使用次数。

举例，小菩萨拥有50%蓝卡耐性提升，此时如果给她30%蓝卡耐性下降以及20%防御力下降（3次）并使用奥伯龙宝具和两张蓝卡，
此时这两张蓝卡的回收只会受到30%蓝卡耐性下降的影响并获得比正常攻击更高的回收，同时20%防御力下降会被消耗完次数。
